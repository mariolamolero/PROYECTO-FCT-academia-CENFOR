...
//ESTA ES MIA
function validar_correo(String $mail, string $campo, array &$errores, $requerido = true):bool
{
    //no ña uso porque la fncion filter_var ya lo tiene en cuenta
    // if(!$requerido && empty($mail)){
    //     return true;
    // }
    $resultado = filter_var($mail, FILTER_VALIDATE_EMAIL); //da false (no imprime nada) o string (true)
...
//ESTA ES MIA
function validarEnlaceYouTube(string $url, string $campo, array &$errores ) {
    // Expresión regular para validar el enlace de YouTube
    $pattern = '/^(https:\/\/youtu\.be\/[A-Za-z0-9_-]{11})(\?si=[A-Za-z0-9_-]+)?$/';
